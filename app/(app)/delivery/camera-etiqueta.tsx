...
export default function CameraEtiquetaScreen() {
  const router = useRouter();
  const setFotoEtiqueta = useDeliveryStore(state => state.setFotoEtiqueta);
  const fotoEtiqueta = useDeliveryStore(state => state.fotoEtiqueta);
  ...
  const takePhoto = async () => {
    ...
    if (!result.canceled && result.assets[0]) {
      setFotoEtiqueta(result.assets[0].uri);
    }
  };
  ...
  const handleNext = () => {
    if (!fotoEtiqueta) return;
    router.push('/(app)/delivery/camera-nf');
  };

  return (
    <View style={styles.container}>
      <Text style={styles.instructions}>Fotografe a etiqueta da{'\n'}Nota Fiscal / DACTE</Text>
      
      <View style={styles.cameraBox}>
        {fotoEtiqueta ? (
          <Image source={{ uri: fotoEtiqueta }} style={styles.preview} />
        ) : (
          <View style={styles.placeholder}>
            <MaterialCommunityIcons name="barcode-scan" size={64} color="#333" />
          </View>
        )}
      </View>
      ...
      <View style={styles.footer}>
        <TouchableOpacity 
          style={[styles.btnPrimary, !fotoEtiqueta && styles.btnDisabled]} 
          onPress={handleNext}
          disabled={!fotoEtiqueta}
        >
